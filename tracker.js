...
let currentUser = typeof getFromStorage === 'function' 
    ? getFromStorage('currentUser', { email: 'guest' }) 
    : (JSON.parse(localStorage.getItem('currentUser')) || { email: 'guest' });
const profileKey = `profile_${currentUser.email}`;
const profile = typeof getFromStorage === 'function'
    ? getFromStorage(profileKey, {})
...
// Use the existing currentUser object from above, don't redefine it
const userKey = `planner_${currentUser.email}`;
let weeklyCalories = typeof getFromStorage === 'function' 
    ? getFromStorage(`${userKey}_calories`) 
    : JSON.parse(localStorage.getItem(`${userKey}_calories`));
let weeklyExercise = typeof getFromStorage === 'function'
    ? getFromStorage(`${userKey}_exercise`)
    : JSON.parse(localStorage.getItem(`${userKey}_exercise`));
let weeklyFoods = typeof getFromStorage === 'function'
    ? getFromStorage(`${userKey}_foods`)
    : JSON.parse(localStorage.getItem(`${userKey}_foods`));
...
function renderSavedFoods(filter = 'all', search = '') {
    ...
    if (emptyState) {
        if (filteredFoods.length === 0) {
            ...
            // Create a more helpful message if they have no saved foods at all
            if (savedFoods.length === 0) {
                emptyState.innerHTML = `
                    <div class="empty-state">
                        <i class="fas fa-heart" style="font-size: 48px; margin-bottom: 15px; color: var(--accent-color);"></i>
                        <h3>No favorite foods saved yet</h3>
                        <p>To save a food as a favorite:</p>
                        <ol>
                            <li>Select a food from the database or enter custom food details</li>
                            <li>Click the "Save to Favorites" button</li>
                            <li>Your favorites will appear here for quick access</li>
                        </ol>
                    </div>
                `;
            }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // This is critical for menu functionality and login validation
    if (typeof window.initializePage === 'function') {
        ...
        // Fallback to directly setting up the mobile menu
        if (typeof window.setupMobileMenu === 'function') {
            console.log("Fallback to setupMobileMenu");
            window.setupMobileMenu();
        }
    }
    
    // Attempt to reinitialize critical elements after a small delay
    // This helps ensure all scripts have loaded properly
    setTimeout(function() {
        console.log("Delayed initialization of critical elements");
        
        // Ensure ingredient select is initialized
        const ingredientSelect = document.getElementById('ingredient');
        if (ingredientSelect && typeof populateIngredients === 'function') {
            console.log("Reinitializing ingredient select");
            populateIngredients();
        ...
        // Ensure Add Ingredient button has event listener
        const addIngredientBtn = document.getElementById('addIngredient');
        if (addIngredientBtn) {
            console.log("Ensuring Add Ingredient button has listener");
            addIngredientBtn.addEventListener('click', addIngredientToMeal);
        }
        
        // Ensure Save to Favorites button has event listener
        const saveToFavoritesBtn = document.getElementById('saveToFavorites');
        if (saveToFavoritesBtn) {
            console.log("Ensuring Save to Favorites button has listener");
            saveToFavoritesBtn.addEventListener('click', saveAsFavorite);
        }
        
        // Ensure Save Custom to Favorites button has event listener
        const saveCustomToFavoritesBtn = document.getElementById('saveCustomToFavorites');
        if (saveCustomToFavoritesBtn) {
            console.log("Ensuring Save Custom to Favorites button has listener");
            saveCustomToFavoritesBtn.addEventListener('click', saveCustomAsFavorite);
        }
        
        // Ensure food tabs are working
        const foodTabs = document.querySelectorAll('.food-tab');
        foodTabs.forEach(tab => {
            tab.addEventListener('click', function() {
                // Get the target tab
                const tabId = this.getAttribute('data-tab');
                console.log("Food tab clicked:", tabId);
                ...
                if (selectedContent) {
                    ...
                    console.error(`Target content ${tabId} not found`);
                }
                
                // Update active state
                foodTabs.forEach(ft => ft.classList.remove('active'));
    ...
    if (ingredientSelect) {
        ...
        try {
            ...
            console.error("Error setting up ingredient select:", error);
        }
    } else {
        console.error("Ingredient select element not found!");
    }
    ...
    console.log(`Found ${tabButtons.length} tab buttons to initialize`);
        ...
        button.addEventListener('click', function() {
            ...
            if (this.closest('.main-tabs')) {
                ...
                if (targetElement) {
                    ...
                    console.error(`Target tab ${tabId} element not found!`);
                }
                ...
                const targetElement = parentTab.querySelector(`#${tabId}`);
                if (targetElement) {
                    targetElement.style.display = 'block';
                } else {
                    console.error(`Target sub-tab ${tabId} element not found!`);
                }
                
                // Update active state
                const subTabButtons = this.parentElement.querySelectorAll('.tab-button');
                ...
                // If My Meals tab is active, render saved foods
                if (tabId === 'my-meals') {
                    if (typeof renderSavedFoods === 'function') {
                        console.log("Rendering saved foods");
                        renderSavedFoods();
                    } else {
                        console.error("renderSavedFoods function not found!");
                    }
                }
            }
        });
...
// Ensure food buttons are properly initialized - this is a final safety check
window.addEventListener('load', function() {
    console.log("Window load event - initializing food tracker functionality");
    
    // Reinitialize the Add Ingredient button
    const addIngredientBtn = document.getElementById('addIngredient');
    if (addIngredientBtn) {
        console.log("Re-attaching event listener to Add Ingredient button");
    ...
    // Reinitialize the Save to Favorites buttons
    const saveToFavoritesBtn = document.getElementById('saveToFavorites');
    if (saveToFavoritesBtn) {
        console.log("Re-attaching event listener to Save to Favorites button");
        // Remove existing listeners to avoid duplicates
        const newSaveBtn = saveToFavoritesBtn.cloneNode(true);
        saveToFavoritesBtn.parentNode.replaceChild(newSaveBtn, saveToFavoritesBtn);
    ...
    // Reinitialize the Save Custom to Favorites button
    const saveCustomToFavoritesBtn = document.getElementById('saveCustomToFavorites');
    if (saveCustomToFavoritesBtn) {
        console.log("Re-attaching event listener to Save Custom to Favorites button");
        // Remove existing listeners to avoid duplicates
        const newSaveCustomBtn = saveCustomToFavoritesBtn.cloneNode(true);
        saveCustomToFavoritesBtn.parentNode.replaceChild(newSaveCustomBtn, saveCustomToFavoritesBtn);
        
        // Add fresh event listener
        newSaveCustomBtn.addEventListener('click', function(e) {
            e.preventDefault();
            console.log("Save Custom to Favorites button clicked");
            saveCustomAsFavorite();
        });
    }
    
    // Check if food buttons have click handlers
    const addButtons = document.querySelectorAll('.add-ingredient-btn');
    if (addButtons.length > 0) {
        console.log("Reinitializing food buttons...");
        addButtons.forEach(button => {
            button.addEventListener('click', function() {
                ...
                if (foodName && calories) {
                    addIngredientToMeal(foodName, calories, protein, carbs, fat);
                }
            });
        });
    }
    
    // Ensure meal type tabs have handlers
    ...
    if (mealTabs.length > 0) {
        mealTabs.forEach(tab => {
            tab.addEventListener('click', function() {
                ...
                // Update UI
                const currentMealTypeElement = document.getElementById('currentMealType');
                if (currentMealTypeElement) {
                    currentMealTypeElement.textContent = mealType.charAt(0).toUpperCase() + mealType.slice(1);
                }
                
                // Update the food list for this meal
                if (typeof updateFoodList === 'function') {
                    updateFoodList();
                }
            });
        });
    ...
    // Ensure food tabs have working event listeners
    const foodTabs = document.querySelectorAll('.food-tab');
    if (foodTabs.length > 0) {
        console.log("Reinitializing food tabs...");
        foodTabs.forEach(tab => {
            // Remove existing listeners to avoid duplicates
            ...
            newTab.addEventListener('click', function() {
                console.log("Food tab clicked:", this.getAttribute('data-tab'));
                // Get the target tab
                const tabId = this.getAttribute('data-tab');
    ...
    // Reinitialize the database search functionality
    const searchBox = document.getElementById('ingredientSearch');
    if (searchBox) {
        console.log("Reinitializing ingredient search box");
        // Remove existing listeners to avoid duplicates
    ...
    // Make sure ingredients are populated
    if (typeof populateIngredients === 'function') {
        console.log("Initial population of ingredients dropdown");
        populateIngredients();
    }
    
    // Force initialization of the mobile menu
    if (typeof window.setupMobileMenu === 'function') {
        window.setupMobileMenu();
    }
});
