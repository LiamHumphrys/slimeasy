...
window.buddyConversationHistory = window.buddyConversationHistory || []; // AI Buddy conversation history (mindset & motivation)
window.chefConversationHistory = window.chefConversationHistory || []; // AI Chef conversation history (meals & nutrition)
window.trainerConversationHistory = window.trainerConversationHistory || []; // AI Trainer conversation history (fitness & exercise)

// Flags to determine if AI coaches should proactively suggest content
window.proactiveSuggestionEnabled = window.proactiveSuggestionEnabled !== undefined ? window.proactiveSuggestionEnabled : true;

// Context tracking for better conversation continuity (shared between coaches)
// Note: window.sharedContext is initialized in dashboard.html
window.sharedContext = window.sharedContext || {
...
// Initialize buddy context if not already defined
window.buddyContext = window.buddyContext || {
...
window.chefContext = window.chefContext || {
...
window.trainerContext = window.trainerContext || {
...
function initializeAICoaches() {
    ...
    if (window.proactiveSuggestionEnabled) {
...
function initializeAIBuddy() {
    ...
    // Define sendBuddyMessage function for this context
    window.sendBuddyMessage = function() {
    ...
    aiSendBtn.addEventListener('click', () => window.sendBuddyMessage());
    
    // Send message on Enter key
    aiMessageInput.addEventListener('keydown', function(e) {
        if (e.key === 'Enter') {
            window.sendBuddyMessage();
        }
    });
    
    // Add voice input button if supported by browser
    if ('webkitSpeechRecognition' in window || 'SpeechRecognition' in window) {
        addVoiceInputButton(aiMessageInput, function() { 
            if (typeof window.sendBuddyMessage === 'function') {
                window.sendBuddyMessage(); 
            }
        });
    }
...
function sendAIMessage() {
    ...
    buddyConversationHistory.push({ role: 'user', message: message });
    ...
    setTimeout(() => {
        ...
        // Store in conversation history
        buddyConversationHistory.push({ role: 'buddy', message: response });
...
function generateAIResponse(message) {
    ...
    // Handle setting-related questions
        ...
        if (message.includes('stop') || message.includes('disable') || 
            ...
            window.proactiveSuggestionEnabled = false;
            ...
            window.proactiveSuggestionEnabled = true;
        ...
        switch(conversationContext.lastTopic) {
            ...
            case 'exercise':
                ...
                if (message.includes('easier') || message.includes('beginner')) {
                    ...
                    const lastType = types.find(type => buddyConversationHistory.slice(-3).some(h => 
...
function checkForProactiveSuggestions() {
    ...
    if (!window.proactiveSuggestionEnabled) return;
...
// Define sendBuddyMessage globally for dashboard accessibility
...
window.sendBuddyMessage = sendBuddyMessage;
